...
- ALERTS
	...
	-> So we cannot locate them using selenium locators. We need to use special class that was created for this purpose. --> Alerts
...
-> What is an iframe?
	...
	-> Iframes are used to create independent html sections for usually ads, maps, and sometimes videos.
	-> <iframe> tag is used to create the iframe.
	-> Selenium is able to focus only one thing at a time.
	-> Therefore selenium will not be able to locate any webELements in an iframe, UNTIL we explicitly switch to it.
-> How many ways to switch to an iframe?
	...
	2- byIndex number of the iframe:
		...
		-> We just need to understand which iframe we need to switch to, then we just pass the index number of it.
...
-> After switching to an iframe and doing some actions in it, we need to EXPLICITLY switch Selenium's focus back to the main frame(parent frame, default content)
...
- WINDOWS/TABS
	-> From selenium's perspective, there is no difference between a tab, and a window. It treats both as the same thing.
...
-> What is a window handle?
	-> A unique randomly generated string that belongs to its window.
	-> Similar to SSN being unique to each person, window handles are unique to each window/tab.
	-> Selenium can only focus to one thing at a time.
	-> Therefore we need to switch its focus explicitly to the window that we want to perform any actions.
...
TESTBASE:
	...
	All of these lines can be simplified by creating a TestBase class where we store them and get them(all of the setup, and/or teardown) when we need it.
...
What is hardcoding?
	...
	-> When you hard code, to be able to change the test data you have to go to each and every single test and change the data from there.

The idea is how to keep these values somewhere outside of our sourcecode(tests), but inside of the project itself.
